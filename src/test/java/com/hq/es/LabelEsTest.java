...
public class LabelEsTest {
    ...
    public void CreateIndex(){
        ...
        IndexOperations ops = template.indexOps(Label.class);
        boolean withMapping = ops.createWithMapping();
        System.out.println(withMapping);
    ...
    public void createDocs(){
        List<Label> labels = JSON.parseObject(data, new TypeReference<List<Label>>() {});
        template.save(labels);
    ...
    @Test
    public void querys(){
        IdsQueryBuilder idsQueryBuilder = QueryBuilders.idsQuery().addIds("1-3SKLS29J");
        SearchHits<Label> search = template.search(new NativeSearchQuery(idsQueryBuilder), Label.class);
        List<Label> collect = search.stream().map(SearchHit::getContent).collect(Collectors.toList());
        collect.forEach(System.out::println);

        //词条查询
        TermQueryBuilder termQueryBuilder = QueryBuilders.termQuery("labelDesc", "国有企业");
        HighlightBuilder.Field labelName = new HighlightBuilder.Field("labelDesc").preTags("<123>").postTags("</font>");

        //高亮查询
        NativeSearchQuery build = new NativeSearchQueryBuilder()
                .withQuery(termQueryBuilder)
                .withHighlightFields(labelName)
                .withPageable(PageRequest.of(0, 20)).build();
        SearchHits<Label> search1 = template.search(build, Label.class);
        List<SearchHit<Label>> searchHits = search1.getSearchHits();
        for (SearchHit<Label> searchHit : searchHits) {
            System.out.println(searchHit.getHighlightField("labelDesc"));

        }

        //模糊查询
        NativeSearchQuery build1 = new NativeSearchQueryBuilder()
                .withQuery(QueryBuilders.fuzzyQuery("labelDesc", "国有企业"))
                .withPageable(PageRequest.of(0, 10))
                .withHighlightFields(new HighlightBuilder.Field("labelDesc").preTags("<font>").postTags("</font>"))
                .build();

        SearchHits<Label> search2 = template.search(build, Label.class);
        List<SearchHit<Label>> searchHits1 = search2.getSearchHits();
